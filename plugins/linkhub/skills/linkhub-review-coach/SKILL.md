...
Use this workflow only for an `IN_REVIEW` report when the caller is the assigned reviewer or a company admin. Route a `DRAFT` report to `linkhub-report-coach`; treat a `CLOSED_*` report as read-only. Reply in the user's language and preserve LinkHub names and enum values exactly.
...
- `reviews_close` always requires a fresh, dedicated confirmation immediately before the call.
...
5. Convert the answers into weights in 5-point increments totaling exactly 100%.
...
Call `reviews_getCloseContext` immediately before proposing closure. Show completeness, effective weight total, performance score, recommended outcome, and every readable OTO candidate. Include every candidate exactly once with `stable`, `growing`, or `declining`; use the returned default only when the user has not stated another trajectory.

Show every user-visible closure effect and ask a dedicated final question without exposing raw IDs or tool syntax. Call `reviews_close` only after that confirmation. If the user selects an outcome different from the recommendation, record their reasoning in the reviewer note instead of silently changing it.
